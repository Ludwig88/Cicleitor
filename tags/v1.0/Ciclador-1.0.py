...
class PROCESO(QtCore.QThread):
    ...
    def ReseteoTodas(self):
        global CondSeteo
        global CondGuardado
        CondGuardado=[['a',False,0.0],['b',False,0.0],['c',False,0.0],['d',False,0.0],['e',False,0.0],['f',False,0.0],['g',False,0.0],['h',False,0.0],['i',False,0.0],['j',False,0.0],['k',False,0.0],['l',False,0.0],['m',False,0.0],['n',False,0.0],['o',False,0.0],['p',False,0.0]]
        #            [celda, Activa?, Promediado]
        CondSeteo=[['a',0,0,0,0,0],['b',0,0,0,0,0],['c',0,0,0,0,0],['d',0,0,0,0,0],['e',0,0,0,0,0],['f',0,0,0,0,0],['g',0,0,0,0,0],['h',0,0,0,0,0],['i',0,0,0,0,0],['j',0,0,0,0,0],['k',0,0,0,0,0],['l',0,0,0,0,0],['m',0,0,0,0,0],['n',0,0,0,0,0],['o',0,0,0,0,0],['p',0,0,0,0,0]]
...
class LECTURA(QtCore.QThread):
    ...
    def run(self):
     ...
     global CondSeteo
     global CondPaBarrer
     CondPaBarrer=[['a',0,0],['b',0,0],['c',0,0],['d',0,0],['e',0,0],['f',0,0],['g',0,0],['h',0,0],['i',0,0],['j',0,0],['k',0,0],['l',0,0],['m',0,0],['n',0,0],['o',0,0],['p',0,0]]
     ...
     while True:
         ...
         print recepcion 
         separado=recepcion.split('#',4) #  devuelve lista de lineas = caracter q separa, lineas a hacer (elementos en q separar -1)
         ...
         if separado[0] != '$' or separado[1] != '$' or separado[2] != '$':
             ...
             if CondSeteo[renglon][4]!=0 :
                 ...
                 Tension=int((int(separado[1])*(0.375))-6144) #(-1)*int((int(separado[1])*(3.0/8))-6144)    # conversion de tension (pasa a mV)
                 ...
                 if (Tension >= V_lim_sup or Tension <=V_lim_inf or Tiempo > inicio+T_Max ):
                     ...
                     if CondPaBarrer[renglon][1]+1 > Barridos:
                         ...
                         CondPaBarrer[renglon][1]=CondPaBarrer[renglon][2]=0    #         [celda, barrido (actual), Tinicio]
                         ...
                         CondPaBarrer[renglon][2]=-1   #float(separado[3]) #pongo -1 asi en la proxima recepcion pone 0
                         ...
                         print 'enviando I='+str(CORRIENTE)+' Descarga? '+str(Descarga)+' celda:'+str(celda)
...
class Myform(QtGui.QMainWindow):
 ...
 def PararCelda(self): #adicionar mandar i=0 para cuando para celda usar por setear en 0
     ...
     PorSetear=Celda
     #para finalizar hago barridos de seteo como 0 (el resto me da lo mismo?)
 ...
 def ActCondUi(self):
     ...
     if CondGuardado[reng][2] == 100:
         self.ui.cmbProm.setCurrentIndex(7) ###promediado
     elif CondGuardado[reng][2] == 50:
 ...
 def PlotFinalCapacidades(self, Celda, PesoAnodo):
     ...
     # symbol='o',
     self.Ploteo2.clear()    
     self.Ploteo2.plot(ValCapacidades, symbol='+', pen='r') #
